...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    snapTradeEnabled: !!config.snapTrade.userSecret
  });
});
  ...
  try {
    ...
    if (llmProvider?.isConfigured) {
      try {
        ...
        const userPortfolio = await snapTrade.getTrades()
        const previousIdeas = memory.getLastRun()?.ideas || [];
        const llmIdeas = await generateLLMIdeas(llmProvider, synthesized, delta, previousIdeas, userPortfolio);
        if (llmIdeas) {
