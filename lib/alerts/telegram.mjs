...
export class TelegramAlerter {
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    if (llmProvider?.isConfigured) {
      try {
        const portfolio = await this.snapTrade.getTrades()
...
export function formatToTelegramHTML(text) {
  if (!text) return "";

  // 1. Escape all reserved MarkdownV2 characters first
  // These MUST have a backslash before them: _ * [ ] ( ) ~ ` > # + - = | { } . !
  let escaped = text.replace(/\\/g, '').replace(/[_*[\]()~`>#+\-=|{}.!]/g, '\\$&');

  // 2. Re-apply formatting using MarkdownV2 syntax
  return escaped
    .replace(/^### (.*$)/gim, '*$1*') // Headers become Bold (V2 doesn't have underline)
    .replace(/\\\*\\\*(.*?)\\\*\\\*/g, '*$1*') // Convert escaped **bold** to *bold*
    .replace(/^\\\* /gm, '• ') // Bullets
    .replace(/^---$/gm, '───────────────');
}